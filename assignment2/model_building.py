...
def baseline_model():
    # create model
    pretrained_model = NASNetLarge(weights="imagenet", include_top=False)
    ...
    model = keras.Model(pretrainedInput, output)

    # Compile model
    model.compile(loss=get_weight(), optimizer=Adam(learning_rate=0.0001), metrics=['AUC'])
    return model
...
model.savel("NASNetLarge_model")
# model = keras.models.load_model("NASNetLarge_model")

test_status, status = evaluate(test_x, test_y)
